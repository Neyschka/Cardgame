...
function tableState(
  overrides: Partial<PublicTableState> = {},
): PublicTableState {
  return {
    roomCode: 'FOXY',
...
describe('DisplayApp', () => {
  it('reads the server-injected room code and joins as display with it', async () => {
    ...
    expect(fake.lastSent('joinAsDisplay').payload).toEqual({
      roomCode: 'FOXY',
    });
  });
  ...
  it('shows an error if the join is rejected', async () => {
    ...
    fake
      .lastSent('joinAsDisplay')
      .ack({ ok: false, reason: 'wrong room code' });
