...
**2026-08-21**: The `game-update/` rules migration replaced the interaction model this answer describes. There's no more legal/illegal card distinction (every card is always playable, per the rewritten `docs/game-mechanics.md`), so the select-then-play grid + greyed "dead" cards are gone — tapping a card plays it immediately, routing to a full-screen targeting step only when `needsTarget` is true and more than one opponent is alive. The HP/shield readout is a numeric shield-pip row (0–4), not a boolean badge. `playAgain` chaining (not covered here at all) lets a turn play more than one card. Current behavior lives in `client/src/player/InMatch.tsx`; `spec.md`'s "Player client" section is the current design description. The prototype files this ticket links (`client/prototype-player.html`, `client/src/prototype-player/`) have since been deleted — the real implementation replaced them.
