...
export function createTableServer(options: TableServerOptions): TableServer {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('joinAsDisplay', (input, ack) => {
      ...
      const result =
        roomCode === null ? MALFORMED : table.joinAsDisplay({ roomCode });
      ack(result);
      // The connection-time `tableState` above almost always beats the
      // display's own connect (a `fetch` first, then a listener) — without
      // this, the display sits on "Connecting…" until some other broadcast
      // happens to come along. A join needs its own state, not a race.
      if (result.ok) socket.emit('tableState', table.publicState());
    });
...
if (isEntryPoint) {
  ...
  createTableServer({
    httpServer,
    table: createTable({ roomCode, lanAddress }),
  });
