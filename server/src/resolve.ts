...
function applyEffect(
  ...
  switch (effect.kind) {
    ...
    case 'heal': {
      ...
      out.push({ kind: 'heal', seatId: actor.seatId, amount: actor.hp - before })
      break
    }
    case 'draw': {
...
export function resolveCard(
  ...
  if (!actor) return out
  const target = targetId ? (seats.find((s) => s.seatId === targetId) ?? null) : null
...
export function autoTarget(seats: CombatSeat[], actorId: string): string | null {
  const opponents = living(seats).filter((s) => s.seatId !== actorId)
