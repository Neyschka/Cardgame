...
import { deckOrderedBy, LETHAL_HAND, seededRandom } from './testFixtures'
...
const dealtHand = (player: TestClient) =>
  waitFor(`a dealt hand`, () => player.hands.find((cards) => cards.length === 5))
...
describe('server socket wiring', () => {
  it('broadcasts tableState to every connected socket when a seat is claimed', async () => {
    ...
    expect(seen.seats[0]).toMatchObject({ isHost: true, hp: 10, handCount: 0 })
  ...
  it('plays a targeted card and broadcasts the result to everyone', async () => {
    ...
    const attack = hand.find((card) => card.type === 'Attack')
    ...
    expect(resolved.seats[1]?.hp).toBe(10 - (attack.value ?? 0))
  ...
  it('rejects an illegal play through its own ack, changing nothing', async () => {
    const server = await startServer()
    const display = await server.client()
    const [alice, bob] = await server.seatPlayers('Alice', 'Bob')
    await alice.socket.emitWithAck('start')
    ...
    const next = await waitFor("Alice's play to land", () =>
      display.states.length > broadcastsBefore ? display.states.at(-1) : undefined,
    )
  ...
  it('restores the same seat and hand when a token reconnects inside the timeout', async () => {
    ...
    const restored = await waitFor('the reclaimed seat to get its hand back', () =>
  ...
  it('carries a match through to a winner and reopens the table on newMatch', async () => {
    const server = await startServer({
      table: { shuffleDeck: deckOrderedBy(LETHAL_HAND) },
    })
    const display = await server.client()
    const [alice, bob] = await server.seatPlayers('Alice', 'Bob')
    await alice.socket.emitWithAck('start')
    const hand = await dealtHand(alice)
    const blows: HandCard[] = []
    let damage = 0
    ...
    expect(restarted.seats.slice(0, 2)).toMatchObject([
      { name: 'Alice', hp: 10, eliminated: false, handCount: 5 },
      { name: 'Bob', hp: 10, eliminated: false, handCount: 5 },
    ])
